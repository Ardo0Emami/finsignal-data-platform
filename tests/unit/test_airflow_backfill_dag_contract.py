...
def test_market_backfill_dag_exists_and_uses_reusable_entrypoint() -> None:
    dag_path = Path("airflow/dags/market_backfill_dag.py")

    assert dag_path.exists()

    dag_source = dag_path.read_text(encoding="utf-8")

    assert 'dag_id="market_backfill"' in dag_source
